...
class RegistryLayout:
    ...
    # In demo we generally won't have current/*.joblib; keep for compatibility
...
def load_run_bundle(layout: RegistryLayout, ref: RunRef) -> Dict[str, Any]:
    ...
    model_file = rd / "model.joblib"
    oof_file = rd / "oof.npy"
    test_pred_file = rd / "test_pred.npy"
    ...
    return {
        ...
        # demo schema (for Live Prediction form)
        "defaults": safe_read_json(rd / "defaults.json"),
        "feature_columns": safe_read_json(rd / "feature_columns.json"),

        # presence flags
        "has_model": model_file.exists(),
        "model_path": str(model_file) if model_file.exists() else None,

        # precomputed arrays (for OOF/Error Analysis pages)
        "oof": np.load(oof_file) if oof_file.exists() else None,
        "test_pred": np.load(test_pred_file) if test_pred_file.exists() else None,
