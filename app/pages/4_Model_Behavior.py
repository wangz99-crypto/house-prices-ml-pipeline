...
# --- lightweight “intuition card” for RMSE ---
try:
    train_path = REPO_ROOT / "data" / "raw" / "train.csv"
    if train_path.exists() and SUMMARY_CSV.exists():
        df_train = pd.read_csv(train_path)
        p50 = float(df_train["SalePrice"].median())
...
names = [n for n in names_all if "ridge" not in n.lower()]

if not names:
    st.info("No tree/boosting feature-importance figures found (excluding Ridge).")
else:
...
try:
    ridge_pipe, _ridge_run_dir = _load_latest_pipeline_and_run(DEMO, "ridge")
except Exception as e:
